/* main.js — 完全版（Firestore対応・検索・月集計・動画紐付け・編集削除） */
...
async function createMatch(){
  ...
  const match={
    ...
    videoId,highlights:[],createdAt:new Date().toISOString()
  };
...
function createPlayButton(videoId,timeSec){
  ...
  btn.type="button";btn.className="wide-btn";
...
function renderScoreCards(){
  ...
  filtered.forEach(({it,idx})=>{
    ...
    if(mt==="") mt="未設定";
    if(!groups[key].counts[mt]) groups[key].counts[mt]=0;
  ...
  Object.keys(groups).sort((a,b)=>b.localeCompare(a)).forEach(key=>{
    ...
    groups[key].items.forEach(({it,idx})=>{
      ...
      delBtn.addEventListener("click",async (e)=>{
        ...
        if(!it.id){alert("Firestore のIDが存在しません");return;}
        try{
          ...
          await loadScores();
        }catch(err){console.error(err);alert("Firestore削除に失敗しました");}
      });
...
function openEditModal(index,date,matchType,opponent,place,myScore,opponentScore,highlights){
  ...
  (Array.isArray(highlights)?highlights:[]).forEach(sec=>document.getElementById("hlList").appendChild(createHlItemElement(sec)));
...
function createHlItemElement(sec){const w=document.createElement("div"); w.className="hl-item"; w.style.display="flex"; w.style.alignItems="center"; w.style.gap="8px"; const sp=document.createElement("span"); sp.textContent=`${sec} 秒`; sp.dataset.second=String(sec); const del=document.createElement("button"); del.type="button"; del.textContent="✕"; del.style.border="none"; del.style.background="transparent"; del.style.color="#c00"; del.style.cursor="pointer"; del.addEventListener("click",()=>w.remove()); w.appendChild(sp); w.appendChild(del); return w;}
...
async function saveEditGeneric(){
  ...
  if(!current.id){alert("Firestore のIDがありません（不整合）"); return;}
  ...
  const highlights=[]; Array.from(document.getElementById("hlList").children).forEach(ch=>{const sp=ch.querySelector("span"); if(!sp) return; const n=Number(String(sp.dataset.second||sp.textContent).replace(" 秒","").trim()); if(!isNaN(n)) highlights.push(n);});
  try{
    ...
    await window._firebaseFns.updateDoc(ref,{
      ...
      pkScore:{myPK:Number(document.getElementById("editPkA")?.value||0),opPK:Number(document.getElementById("editPkB")?.value||0)},
      highlights,videoId
    });
    alert("Firestore に保存しました！");
    closeEditModal();
...
async function deleteCurrentMatch(){
  ...
  if(!confirm("この試合を削除しますか？")) return;
  const current=scores[window.currentEditIndex];
  if(!current.id){alert("Firestore のIDが存在しません。削除できません。"); return;}
  try{
    const ref=window._firebaseFns.doc(window._firebaseDB,"scores",current.id);
    await window._firebaseFns.deleteDoc(ref);
    alert("Firestore から削除しました");
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  function togglePkRow(){if(!scoreAEl||!scoreBEl||!pkRow) return; pkRow.style.display=Number(scoreAEl.value||0)===Number(scoreBEl.value||0)?"flex":"none";}
  scoreAEl?.addEventListener("input",togglePkRow);
  scoreBEl?.addEventListener("input",togglePkRow);

  document.getElementById("btnAddYouTube")?.addEventListener("click",()=>{const url=document.getElementById("youtubeUrl")?.value.trim(); if(!url) return alert("URLを入力してください"); addYouTubeVideo(url); document.getElementById("youtubeUrl").value="";});
  ...
  document.getElementById("btnJoin")?.addEventListener("click",async()=>{
    ...
    localStorage.setItem("teamInfo",JSON.stringify({teamName:name,inviteCode:code||null}));

    document.getElementById("teamSection").style.display="none";
    document.getElementById("scoresSection").style.display="block";

    if(isAdmin()){document.getElementById("addVideoSection").style.display="block"; document.getElementById("createMatchSection").style.display="block";}
    else{document.getElementById("addVideoSection").style.display="none"; document.getElementById("createMatchSection").style.display="none";}

    document.getElementById("currentTeamName").textContent=`${name}（招待コード: ${code||"-"}）`;
